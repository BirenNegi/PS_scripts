...
if ($Action -eq 'Install') {
    ...
    Register-ScheduledTask -TaskName $TaskName -Action $taskAction -Trigger $trigger -Settings $settings -Principal $principal -Description "Hyper-V IOPS Monitor" | Out-Null

    Write-Host "`n Task '$TaskName' installed successfully." -ForegroundColor Green
    Write-Host " Runs as    : SYSTEM"
    Write-Host " Trigger    : At system startup"
    Write-Host " Script     : $ScriptPath"
    Write-Host " Threshold  : $ThresholdPercent% of peak IOPS"
